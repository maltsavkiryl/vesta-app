...
function CategoryPicker({
  ...
  return (
    <SegmentedControl
      ...
      onChange={(event) => {
        const nextCategory = categories[event.nativeEvent.selectedSegmentIndex]
        ...
        >
          {uploading ? (
            ...
            <>
              ...
              >
                <View style={[styles.dropIcon, { backgroundColor: `${tokens.accent}14` }]}>
...
function PayslipSheet({ onClose, payslip }: { onClose: () => void; payslip: Payslip | null }) {
  const tokens = useDesignTokens()
        ...
        >
          {payslip ? (
            <>
              ...
              <AppButton label="Download PDF" onPress={onClose} />
            </>
          ) : null}
        </ScrollView>
      </View>
      ...
      <ContractSheet
        ...
        onSign={(contract) => {
          setContracts((current) =>
            current.map((item) => (item.id === contract.id ? { ...item, status: "signed" } : item)),
          )
